...
import type { PrismaClient, SmsStatus } from '@prisma/client';
import { maskPhone } from '@/lib/mask.js';
import type { ListOtpQueryT, ListSmsQueryT } from './admin.schemas.js';

export class AdminController {
  constructor(private readonly prisma: PrismaClient) {}

  listSms = async (
    req: FastifyRequest<{ Querystring: ListSmsQueryT }>,
    reply: FastifyReply,
  ): Promise<void> => {
    ...
    const where = {
      ...(status ? { status: status as SmsStatus } : {}),
      ...(deviceId ? { deviceId } : {}),
      ...(from || to
        ? {
            createdAt: {
  ...
  listOtp = async (
    req: FastifyRequest<{ Querystring: ListOtpQueryT }>,
    reply: FastifyReply,
  ): Promise<void> => {
  ...
  stats = async (req: FastifyRequest, reply: FastifyReply): Promise<void> => {
