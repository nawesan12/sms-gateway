...
export interface CreateAccessLinkOutput {
  link: string;
  userId: string;
  phoneE164: string;
  issuedAt: Date;
...
export class AccessTokenService {
  ...
  async createAccessLink(input: CreateAccessLinkInput): Promise<CreateAccessLinkOutput> {
    ...
    if (user.role !== desiredRole) {
      await this.prisma.user.update({
        where: { id: user.id },
        data: { role: desiredRole as UserRole },
      });
    }
  ...
  async findUserByToken(plain: string): Promise<User | null> {
