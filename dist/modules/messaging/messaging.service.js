...
export class MessagingService {
    ...
    smsQueue;
    tokens;
    constructor(prisma, env, logger, smsService, router, smsQueue, tokens) {
        this.prisma = prisma;
        this.env = env;
        this.logger = logger;
    ...
    async sendOne(input) {
        ...
        if (!reservation.ok) {
            await this.smsService.finalizeFailure(smsId, ERROR_CODES.INSUFFICIENT_TOKENS, 'no token balance');
        ...
        try {
            ...
            await this.smsService.finalizeFailure(smsId, 'ENQUEUE_FAILED', err.message);
            throw err;
        }
        this.logger.info({
            smsMessageId: smsId,
    ...
    async getSmsStatus(id) {
        ...
        if (!sms)
            throw new AppError(ERROR_CODES.NOT_FOUND, 'SMS not found', 404);
        return sms;
    }
}
